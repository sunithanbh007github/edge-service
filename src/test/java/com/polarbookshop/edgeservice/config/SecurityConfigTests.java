...
public class SecurityConfigTests {
    ...
    void whenLogoutAuthenticatedAndWithCsrfTokenThen302() {
        ...
        webClient
                .mutateWith(SecurityMockServerConfigurers.mockOidcLogin())
                .mutateWith(SecurityMockServerConfigurers.csrf()) //Enhances the request to provide the required CSRF token
                .post()
                .uri("/logout")
                .exchange()
                .expectStatus().isFound(); //The response is a redirect to Keycloak to propagate the logout operation.

    }

    private ClientRegistration testClientRegistration() {
        return ClientRegistration.withRegistrationId("test") //A mock ClientRegistration used by Spring Security to get the URLs to
                                                             // contact Keycloak
